...
# ★ نفس جولة التطبيق، بس على الناتج المنشور — يلي بتنشال منه التعليقات.
# بلا هالخطوة، كسر بالماسح بيوصل للمستخدم وحده: الاختبارات بتشتغل على
# المصدر، والمستخدم بيفتح الناتج.
echo "▸ تطبيق الطلاب (على الناتج المنشور، بلا تعليقات)"
./tools/build_dist.sh >/dev/null
SERVE_FROM=dist node tests/browser.mjs

echo "▸ ماسح التعليقات"
node tests/strip.mjs
...
# حكم واحد بالآخر.
#
# سببه خطأ حقيقي صار: قصّيت مخرجات الجولة بـgrep وشفت «✓ كل الـ…» من
# مرحلة، وحسبتها نجحت — ومرحلة بعدها فشلت وانرفع الكود ومرق للـCI.
# set -e بيوقف الجولة عند أول فشل، فوصول التنفيذ لهون معناه إن كل
# المراحل مرقت. سطر واحد ما بينقصّ بالغلط.
# ---------------------------------------------------------------------
echo
echo "════════════════════════════════════════"
echo "  ✓ كل المراحل نجحت"
echo "════════════════════════════════════════"
